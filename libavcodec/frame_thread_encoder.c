...
#if 1	
	av_log(avctx, AV_LOG_ERROR,"##context##");
	for( j = 0; j < sizeof(AVCodecContext); j+=4 )
	{
		if(j%32==0) av_log(avctx, AV_LOG_ERROR,"\n");
		av_log(avctx, AV_LOG_ERROR," %02x%02x%02x%02x",p[j], p[j+1], p[j+2], p[j+3]);
	}
